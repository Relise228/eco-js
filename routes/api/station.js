...
router.post("/unitsFull", auth, async (req, res) => {
  ...
  connection.on("connect", function (err) {
    var all = []
    request = new Request(
      `select Station_Units.ID_Measured_Unit, Title, Unit
                                      from Station_Units inner join Measured_Unit
                                      ON Station_Units.ID_Measured_Unit = Measured_Unit.ID_Measured_Unit
...
router.post("/units/", auth, async (req, res) => {
  ...
  connection.on("connect", function (err) {
    var all = []
    request = new Request(
      `select Title from Station_Units inner join Measured_Unit ON Station_Units.ID_Measured_Unit = Measured_Unit.ID_Measured_Unit where ID_Station = '${ID_Station}' ORDER BY Title;`,
      function (err, rowCount, rows) {
        connection.close()
        if (err) {
          console.log(err)
...
router.post("/fromFavorite", auth, async (req, res) => {
  ...
  connection.connect()
  connection.on("connect", function (err) {
    request = new Request(`delete from Favorite_Station where ID_User = ${req.user.id} and ID_Station = '${ID_Station}';`, function (
      err,
      rowCount,
      rows
    ) {
