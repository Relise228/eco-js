...
router.post('/toFavorite', auth, async (req, res) => {
  ...
  connection.on('connect', function (err) {
    request = new Request(
      `IF NOT EXISTS (select * from Favorite_Station where ID_Station = '${ID_Station}' and ID_User = ${req.user.id}) insert into Favorite_Station(ID_User, ID_Station) values (${req.user.id}, '${ID_Station}');`,
      function (err, rowCount, rows) {
        connection.close();
        if (err) {
          console.log(err);
...
router.post('/fromFavorite', auth, async (req, res) => {
  ...
  connection.on('connect', function (err) {
    request = new Request(
      `delete from Favorite_Station where ID_User = ${req.user.id} and ID_Station = '${ID_Station}';`,
      function (err, rowCount, rows) {
        connection.close();
        if (err) {
          console.log(err);
...
router.get('/', auth, async (req, res) => {
  ...
  connection.on('connect', function (err) {
    var all = [];
    let requestStr = `select ID_Station, Name, Status, ID_Server, ID_SaveEcoBot, Longitude, Latitude, 
        (select Favorite_Station.ID_Station from Favorite_Station where ID_User = ${req.user.id} 
        and Favorite_Station.ID_Station = Station_Coordinates.ID_Station) as Favorite from Station_Coordinates`;
    let favStr = '';
    let orderStr = '';
    ...
    if (url.onlyFav) {
      if (url.onlyFav == 'true') {
        favStr =
          ...
          ` exists (select * from Favorite_Station where ID_User = ${req.user.id} 
                    and Favorite_Station.ID_Station = Station_Coordinates.ID_Station)`;
      }
    }
    ...
    request = new Request(requestStr + favStr + orderStr, function (
      err,
      rowCount,
      rows
    ) {
